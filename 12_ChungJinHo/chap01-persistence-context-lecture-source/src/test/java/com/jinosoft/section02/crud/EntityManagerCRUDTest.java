package com.jinosoft.section02.crud;
...
class EntityManagerCRUDTest {
  ...
  @DisplayName("메뉴코드로 메뉴 조회( SELECT)")
  @ParameterizedTest
  ...
  private static Stream<Arguments> newMenu(){
    return Stream.of(Arguments.of("아구찜",40000,4,"Y"));
  }

  @DisplayName("새로운 메뉴 추가")
  @ParameterizedTest
  @MethodSource("newMenu")
  void saveAndReturnAllCount(String menuName, int price, int menuCode, String oderableStatus) {
    // when
    Menu newMenu = new Menu(menuName, price, menuCode, oderableStatus);
    Long count = entityManagerCRUD.saveAndReturnAllCount(newMenu);
    // then
    System.out.println("before insert count : " + count);
    assertNotNull(newMenu);
    assertEquals(22 + 1, count);
    System.out.println("after insert count : " + count);
  ...
  @ParameterizedTest
  @ValueSource(ints = {100}) // 100이라는 정수 값을 테스트 메서드의 매개변수로 전달
